...
function QARenderer.buildCell(action_id, spec)
    ...
    local is_text_only = (spec.shape == "text_only")

    local frame_widget
    local text_only_w
    if is_text_only then
        -- 纯文字 cell:宽度由文字内容本身决定,不强制撑满 frame_sz
        -- （和参考实现里 tw_size.w 的用法一致),这样多个 text_only
        -- cell 排成一行时不会各自占满等宽的格子。
        frame_widget = TextWidget():new{
            text                   = entry.label,
            face                   = Font():getFace(spec.lbl_face or style.FACE_REGULAR, spec.lbl_fs or 15),
            fgcolor                = fgcolor,
            max_width              = spec.lbl_max_width or frame_sz,
            truncate_with_ellipsis = true,
        }
        text_only_w = frame_widget:getSize().w
    else
        local icon_widget = QARenderer.buildIcon(entry, spec.icon_sz, fgcolor, spec.icon_opts)
        frame_widget = QARenderer.buildFrame(icon_widget, frame_sz, {
            shape      = spec.shape,
            bg         = spec.bg,
    ...
    local col_h = is_text_only and frame_widget:getSize().h or frame_sz

    -- text_only 模式下 label 本身就是内容,不再叠加下面这份 label。
    if spec.show_label and not is_text_only then
    ...
    if spec.on_tap_fn then
        local tap_w = is_text_only and text_only_w or frame_sz
        cell_widget = _buildTapContainer(col, tap_w, col_h, action_id, spec, "TapQACell")
    end

    return cell_widget, frame_widget
end

-- Builds a "丨" separator for use *between* consecutive text_only cells in
-- a row (see buildCell's "text_only" shape). Deliberately not something
-- buildCell inserts itself: buildCell only ever builds one action's cell
-- per call, so it has no idea whether this is the first item in the row or
-- what gap spacing the row wants — that's row-assembly state that only the
-- caller looping over multiple action_ids has (its own `i`, its own
-- HorizontalGroup). Mirrors the "丨" + optional HorizontalSpan padding from
-- the original inline implementation.
--
-- spec fields:
--   lbl_face, lbl_fs — match the text_only cells it sits between.
--   fgcolor          — separator color (pass a muted/secondary color so it
--     reads as a divider, not another action).
--   gap_px           — horizontal padding on each side of the glyph. When
--     0/nil, the glyph is returned bare with no padding.
function QARenderer.buildTextSeparator(spec)
    spec = spec or {}
    local style = SUIStyle()
    ...
    local sep_text = TextWidget():new{
        text    = "丨",
        face    = Font():getFace(spec.lbl_face or style.FACE_REGULAR, spec.lbl_fs or 15),
        fgcolor = spec.fgcolor or style.COLOR.text_secondary,
    }

    if spec.gap_px and spec.gap_px > 0 then
        local group = HorizontalGroup():new{ align = "center" }
        group[#group + 1] = HorizontalSpan():new{ width = spec.gap_px }
