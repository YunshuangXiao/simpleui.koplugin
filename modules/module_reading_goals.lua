...
function M.getHeight(_ctx)
    ...
    local h = 0
    if isCompact() then
        h = _compactRowsHeight(n, _compactDims(Config.getModuleScale("reading_goals", pfx) * lf))
    else
        local d = _scaledDims(Config.getModuleScale("reading_goals", pfx) * lf)
